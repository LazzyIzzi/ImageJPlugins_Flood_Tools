...
public class Flood_Connectivity_V3_Toggle implements PlugInFilter
{
	ImagePlus imp;
	...
	{		
		...
		GenericDialog gd = new GenericDialog("Flood Connectivity Scan");
		gd.setInsets(0,0,0);
		...
		//gd.addCheckbox("Characterize Image", true);
		gd.addCheckbox("Convert to hybrid map:", true);
		gd.addCheckbox("Compute Tortuosity", true);
	...
	public void run(ImageProcessor ip)
	{
		...
		if(dp!=null)
		{
			if(ValidateParams(dp))
			{
				...
				FloodReport fldRpt = hff.hybridFloodFill(oImageArr,w,h,d,pw,ph,pd,pu,dp.floodMin, dp.neighbors,dp.doEDM,dp.doGDT);
